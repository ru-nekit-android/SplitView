...
public class AbstractUPCEANWriter implements UPCEANWriter 
{
  ...
  protected static function renderResult(code:Array, width:int, height:int):ByteMatrix  {
	const inputWidth:int = code.length;
    // Add quiet zone on both sides
	const fullWidth:int = inputWidth + (AbstractUPCEANReader.START_END_PATTERN.length << 1);
	const outputWidth:int = Math.max(width, fullWidth);
	const outputHeight:int = Math.max(1, height);

	const multiple:int = outputWidth / fullWidth;
	const leftPadding:int = (outputWidth - (inputWidth * multiple)) / 2;

    var output:ByteMatrix = new ByteMatrix(outputWidth, outputHeight);
	const outputArray:Vector.<Vector.<int>> = output.getArray();

...
package com.google.zxing.qrcode
{
	...
	{ 
		...
		private static function renderResult( code:QRCode, width:int, height:int):ByteMatrix {
			...
			const inputWidth:int = input.width();
			const inputHeight:int = input.height();
			const qrWidth:int = inputWidth + (QUIET_ZONE_SIZE << 1);
			const qrHeight:int = inputHeight + (QUIET_ZONE_SIZE << 1);
			const outputWidth:int = Math.max(width, qrWidth);
			const outputHeight:int = Math.max(height, qrHeight);
			...
			const leftPadding:int = (outputWidth - (inputWidth * multiple)) / 2;
			const topPadding:int = (outputHeight - (inputHeight * multiple)) / 2;
			
			var output:ByteMatrix = new ByteMatrix(outputHeight, outputWidth);
			var outputArray:Vector.<Vector.<int>> = output.getArray(); //sbyte[][]
			...
			var inputArray:Vector.<Vector.<int>> = input.getArray();
		...
		private static function setRowColor(row:Vector.<int>, value:int):void 
		{

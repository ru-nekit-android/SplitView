package com.google.zxing.qrcode.encoder
{
	...
	public class MaskUtil
	{
		...
		{
			var penalty:int = 0;
			var array:Vector.<Vector.<int>> = matrix.getArray(); //sbyte[][]
		...
		{
			var penalty:int = 0;
			var array:Vector.<Vector.<int>> = matrix.getArray();
			var width:int = matrix.width();
			var height:int = matrix.height();
		...
		private static function applyMaskPenaltyRule1Internal(matrix:ByteMatrix,isHorizontal:Boolean):int
		{
			...
			const iLimit:int = isHorizontal ? matrix.height() : matrix.width();
			const jLimit:int = isHorizontal ? matrix.width() : matrix.height();
			const array:Vector.<Vector.<int>> = matrix.getArray(); // sbyte[][]
